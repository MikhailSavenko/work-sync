...
from drf_yasg.openapi import TYPE_ARRAY, TYPE_OBJECT, TYPE_STRING
...
from task.doc.texts import COMMENT_TEXTS, EVALUATION_TEXTS, TASK_TEXTS
    ...
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "list":
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=task_text_create, status_code=403
            )
            operation.responses["400"] = OpenApiResponse(
                task_text_create["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"name_field": Schema(type=TYPE_STRING)},
                    example=task_text_create["example"]["400"],
                ),
            )
            ...
            operation.responses["404"] = OpenApiResponse(
                task_text_read["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=task_text_read["example"]["404"],
                ),
            )
            ...
            operation.responses["400"] = OpenApiResponse(
                ...
                Schema(
                    type=TYPE_OBJECT,
                    properties={"name_field": Schema(type=TYPE_STRING)},
                    example=task_text_update["example"]["400"],
                ),
            )
            operation.responses["409"] = OpenApiResponse(
                task_text_update["responses"]["409"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"task_update_conflict": Schema(type=TYPE_STRING)},
                    example=task_text_update["example"]["409"]["task_update_conflict"],
                ),
            )
            ...
            operation.responses["409"] = OpenApiResponse(
                ...
                Schema(
                    type=TYPE_OBJECT,
                    properties={"task_update_conflict": Schema(type=TYPE_STRING)},
                    example=task_text_partial_upd["example"]["409"]["task_update_conflict"],
                ),
            )
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=task_text_delete, status_code=403
            )
            operation.responses["404"] = OpenApiResponse(
                task_text_delete["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=task_text_delete["example"]["404"],
                ),
            )
...
class CommentAutoSchema(SwaggerAutoSchema):
    ...
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "list":
            ...
            operation.responses["404"] = OpenApiResponse(
                comment_text_list["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=comment_text_list["example"]["404"],
                ),
            )
            operation.responses["400"] = OpenApiResponse(
                comment_text_list["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=comment_text_list["example"]["400"],
                ),
            )
            ...
            operation.responses["404"] = OpenApiResponse(
                comment_text_create["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=comment_text_create["example"]["404"],
                ),
            )
            operation.responses["400"] = OpenApiResponse(
                comment_text_create["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"field_name": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=comment_text_create["example"]["400"],
                ),
            )
            ...
            operation.responses["404"] = OpenApiResponse(
                comment_text_read["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=comment_text_read["example"]["404"],
                ),
            )
            operation.responses["400"] = OpenApiResponse(
                comment_text_read["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=comment_text_read["example"]["400"],
                ),
            )
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=comment_text_partial_upd, status_code=403
            )
            operation.responses["404"] = OpenApiResponse(
                comment_text_partial_upd["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=comment_text_partial_upd["example"]["404"],
                ),
            )
            operation.responses["400"] = OpenApiResponse(
                comment_text_partial_upd["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"name_field": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=comment_text_partial_upd["example"]["400"],
                ),
            )
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=comment_text_delete, status_code=403
            )
            operation.responses["404"] = OpenApiResponse(
                comment_text_delete["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=comment_text_delete["example"]["404"],
                ),
            )
...
class EvaluationAutoSchema(SwaggerAutoSchema):
    ...
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "create":
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=evaluation_text_create, status_code=403
            )
            operation.responses["409"] = OpenApiResponse(
                evaluation_text_create["responses"]["409"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"evaluation_create_conflict": Schema(type=TYPE_STRING)},
                    example=evaluation_text_create["example"]["409"],
                ),
            )
            operation.responses["404"] = OpenApiResponse(
                evaluation_text_create["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=evaluation_text_create["example"]["404"],
                ),
            )
            operation.responses["400"] = OpenApiResponse(
                evaluation_text_create["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=evaluation_text_create["example"]["400"],
                ),
            )
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=evaluation_text_patial_upd, status_code=403
            )
            operation.responses["404"] = OpenApiResponse(
                evaluation_text_patial_upd["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=evaluation_text_patial_upd["example"]["404"],
                ),
            )
            operation.responses["400"] = OpenApiResponse(
                evaluation_text_patial_upd["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=evaluation_text_patial_upd["example"]["400"],
                ),
            )
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=evaluation_text_delete, status_code=403
            )
            operation.responses["404"] = OpenApiResponse(
                evaluation_text_delete["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=evaluation_text_delete["example"]["404"],
                ),
            )
        return operation
