...
class WorkerServiceTestCase(TestCase):
    ...
    def test_get_calendar_events_input_worker_without_events(self):
        result = get_calendar_events(
            worker=self.worker1, start_date=self.DATE_NO_IVENTS_START, end_date=self.DATE_NO_IVENTS_END
        )
    ...
    def test_get_calendar_events_input_worker_with_events(self):
        result = get_calendar_events(
            worker=self.worker0,
            start_date=(self.DATETIME_NOW - self.TIMEDELTA_THREE_DAYS),
            end_date=(self.DATETIME_NOW + self.TIMEDELTA_THREE_DAYS + self.TIMEDELTA_THREE_DAYS),
        )
    ...
    def test_get_evaluations_avg_with_evaluation(self):
        result = get_evaluations_avg(
            worker=self.worker0,
            start=(self.DATETIME_NOW - self.TIMEDELTA_THREE_DAYS),
            end=(self.DATETIME_NOW + self.TIMEDELTA_THREE_DAYS),
        )
