...
from drf_yasg.openapi import IN_BODY, IN_PATH, TYPE_ARRAY, TYPE_INTEGER, TYPE_OBJECT, TYPE_STRING, Parameter
from drf_yasg.openapi import Response as OpenApiResponse
from drf_yasg.openapi import Schema

from account.doc.texts import (
    TEAM_TEXTS,
    ...
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "list":
            ...
            operation.responses["404"] = OpenApiResponse(
                team_text_read["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING, description=team_text_read["responses"]["404"])},
                    example=team_text_read["example"]["404"],
                ),
            )
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=team_text_create, status_code=403
            )
            operation.responses["400"] = OpenApiResponse(
                team_text_create["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"name_field": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=team_text_create["example"]["400"],
                ),
            )
            operation.responses["409"] = OpenApiResponse(
                team_text_create["responses"]["409"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING, description=team_text_create["responses"]["409"])},
                    example=team_text_create["example"]["409"],
                ),
            )
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=team_text_update, status_code=403
            )
            operation.responses["400"] = OpenApiResponse(
                team_text_update["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"name_field": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
            ...
            operation.responses["409"] = OpenApiResponse(
                ...
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING, description=team_text_update["responses"]["409"])},
                    example=team_text_update["example"]["409"],
                ),
            )
            ...
            operation.responses["403"] = get_response_open_api_scheme_with_detail_string_and_example(
                text=team_text_delete, status_code=403
            )
            operation.responses["404"] = OpenApiResponse(
                team_text_delete["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING, description=team_text_delete["responses"]["404"])},
                    example=team_text_delete["example"]["404"],
                ),
            )
        return operation
...
class WorkerAutoSchema(SwaggerAutoSchema):
    ...
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "list":
            ...
            operation.responses["404"] = OpenApiResponse(
                worker_text_read["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING, description=worker_text_read["responses"]["404"])},
                    example=worker_text_read["example"]["404"],
                ),
            )
            ...
            operation.responses["400"] = OpenApiResponse(
                ...
                Schema(
                    type=TYPE_OBJECT,
                    properties={"name_field": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=worker_text_partial_upd["example"]["400"],
                ),
            )
            ...
            operation.responses["404"] = OpenApiResponse(
                worker_text_calendar_day["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={
                        "detail": Schema(type=TYPE_STRING, description=worker_text_calendar_day["responses"]["404"])
                    },
                    example=worker_text_calendar_day["example"]["404"],
                ),
            )
            ...
            operation.responses["404"] = OpenApiResponse(
                worker_text_calendar_month["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={
                        "detail": Schema(type=TYPE_STRING, description=worker_text_calendar_month["responses"]["404"])
                    },
                    example=worker_text_calendar_month["example"]["404"],
                ),
            )
            ...
            operation.responses["404"] = OpenApiResponse(
                worker_text_average_evaluation["responses"]["404"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={
                        "detail": Schema(
                            type=TYPE_STRING, description=worker_text_average_evaluation["responses"]["404"]
                        )
                    },
                    example=worker_text_average_evaluation["example"]["404"],
                ),
            )
        return operation
    ...
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "create":
            ...
            operation.responses["200"] = OpenApiResponse(
                token_obtain_create["responses"]["200"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"access": Schema(type=TYPE_STRING), "refresh": Schema(type=TYPE_STRING)},
                    example=token_obtain_create["example"]["200"],
                ),
            )
            operation.responses["401"] = OpenApiResponse(
                token_obtain_create["responses"]["401"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING)},
                    example=token_obtain_create["example"]["401"],
                ),
            )
        return operation
...
class TokenBlacklistAutoSchema(SwaggerAutoSchema):
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "create":
            ...
            operation.responses["401"] = OpenApiResponse(
                token_blacklist_create["responses"]["401"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING), "code": Schema(type=TYPE_STRING)},
                    example=token_blacklist_create["example"]["401"],
                ),
            )
        return operation
...
class TokenRefreshAutoSchema(SwaggerAutoSchema):
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "create":
            ...
            operation.responses["200"] = OpenApiResponse(
                token_refresh_create["responses"]["200"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"access": Schema(type=TYPE_STRING), "refresh": Schema(type=TYPE_STRING)},
                    example=token_refresh_create["example"]["200"],
                ),
            )
            operation.responses["401"] = OpenApiResponse(
                token_refresh_create["responses"]["401"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING), "code": Schema(type=TYPE_STRING)},
                    example=token_refresh_create["example"]["401"],
                ),
            )
        return operation
...
class TokenVerifyAutoSchema(SwaggerAutoSchema):
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "create":
            ...
            operation.responses["200"] = OpenApiResponse(
                token_verify_create["responses"]["200"],
                Schema(type=TYPE_OBJECT, properties={}, example=token_verify_create["example"]["200"]),
            )
            operation.responses["401"] = OpenApiResponse(
                token_verify_create["responses"]["401"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"detail": Schema(type=TYPE_STRING), "code": Schema(type=TYPE_STRING)},
                    example=token_verify_create["example"]["401"],
                ),
            )
            operation.responses["400"] = OpenApiResponse(
                token_verify_create["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"non_field_errors": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=token_verify_create["example"]["400"],
                ),
            )
        return operation
...
class UserAutoSchema(SwaggerAutoSchema):
    def get_operation(self, operation_keys=None):
        ...
        if operation_keys and operation_keys[-1] == "create":
            ...
            operation.responses["201"] = OpenApiResponse(
                user_register_create["responses"]["201"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"email": Schema(type=TYPE_STRING), "id": Schema(type=TYPE_INTEGER)},
                    example=user_register_create["example"]["201"],
                ),
            )
            operation.responses["400"] = OpenApiResponse(
                user_register_create["responses"]["400"],
                Schema(
                    type=TYPE_OBJECT,
                    properties={"name_field": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                    example=user_register_create["example"]["400"],
                ),
            )
            ...
            if action == "update":
                ...
                operation.responses["400"] = OpenApiResponse(
                    user_me_update["responses"]["400"],
                    Schema(
                        type=TYPE_OBJECT,
                        properties={"name_field": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                        example=user_me_update["example"]["400"],
                    ),
                )
                ...
                scheme_current_password = Schema(
                    type=TYPE_OBJECT, properties={"current_password": Schema(type=TYPE_STRING)}
                )
                operation.parameters.append(Parameter("data", IN_BODY, required=True, schema=scheme_current_password))
                operation.responses["204"] = OpenApiResponse(user_me_delete["responses"]["204"])
                operation.responses["401"] = OpenApiResponse(
                    user_me_delete["responses"]["401"],
                    Schema(
                        type=TYPE_OBJECT,
                        properties={"detail": Schema(type=TYPE_STRING), "code": Schema(type=TYPE_STRING)},
                        example=user_me_delete["example"]["401"],
                    ),
                )
                operation.responses["400"] = OpenApiResponse(
                    user_me_delete["responses"]["400"],
                    Schema(
                        type=TYPE_OBJECT,
                        properties={"name_field": Schema(type=TYPE_ARRAY, items=Schema(type=TYPE_STRING))},
                        example=user_me_delete["example"]["400"],
                    ),
                )
