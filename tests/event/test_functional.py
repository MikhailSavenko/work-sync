...
class MeetingApiTestCase(ApiTestCaseBase):
    ...
    def setUpTestData(cls):
        ...
        cls.meeting_conflict_data = {
            ...
            "workers": [cls.worker_normal2.id],
        }

        cls.meeting_no_conflict_data = {
            "description": "My Meeting",
            "datetime": cls.meeting1.datetime.strftime("%Y-%m-%dT%H:%M"),
            "workers": [cls.worker_normal1.id, cls.worker_normal3.id],
        }
        ...
        for user in self.user_role_all:
            with self.subTest(user=user):
                ...
                response = self.client.post(
                    reverse("event:meeting-list"), data=self.meeting_no_valid_data, format="json"
                )
    ...
    def test_update_input_valid_data_meeting(self):
        ...
        response = self.client.put(
            reverse("event:meeting-detail", kwargs={"pk": self.meeting.id}), data=self.meeting_valid_data
        )
    ...
    def test_update_no_valid_data_meeting(self):
        self.client.force_authenticate(user=self.user_normal)
        response = self.client.put(
            reverse("event:meeting-detail", kwargs={"pk": self.meeting.id}), data=self.meeting_no_valid_data
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertCountEqual(["description", "datetime", "workers"], response.json())
    ...
    def test_update_conflict_meeting(self):
        ...
        response = self.client.put(
            reverse("event:meeting-detail", kwargs={"pk": self.meeting1.id}), data=self.meeting_conflict_data
        )
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
        self.assertCountEqual(["detail"], response.json())
    ...
    def test_update_no_conflict_if_this_your_meeting(self):
        self.client.force_authenticate(user=self.user_normal1)
        response = self.client.put(
            reverse("event:meeting-detail", kwargs={"pk": self.meeting1.id}), data=self.meeting_no_conflict_data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
