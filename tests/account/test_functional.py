...
from tests.factories import EvaluationFactory, MeetingFactory, TaskFactory, TeamFactory, UserFactory
...
class TeamApiTestCase(ApiTestCaseBase):
    ...
    def setUpTestData(cls):
        ...
        cls.data_team = {"title": "Test Team", "description": "Test descript =)", "workers": [cls.worker_admin.id]}
        cls.update_data_team = {
            "title": "Test Team Update",
            "description": "Test descript Update =)",
            "workers": [cls.worker_admin.id],
        }
    ...
    def test_check_team_conflict_update_team_pk_not_none(self):
        ...
        response = self.client.put(
            reverse("account:teams-detail", kwargs={"pk": self.team.id}), data=self.data_team_conflict, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
    ...
    def test_admin_can_update_his_team(self):
        self.client.force_authenticate(user=self.user_admin)
        response = self.client.put(
            reverse("account:teams-detail", kwargs={"pk": self.team.id}), data=self.update_data_team, format="json"
        )
...
class WorkerApiTestCase(ApiTestCaseBase):
    ...
    def setUpTestData(cls):
        ...
        cls.role_no_valid_data = {"role": "MN"}
        cls.role_manager_data = {"role": "MG"}
        ...
        for user in self.user_role_all:
            with self.subTest(user=user):
                ...
                response = self.client.get(
                    reverse(
                        "account:worker-average-evaluation",
                        kwargs={"pk": self.worker_normal.id, "start_date": start_date, "end_date": end_date},
                    )
                )
                self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_calendar_day_worker(self):
        for user in self.user_role_all:
            with self.subTest(user=user):
                ...
                response = self.client.get(
                    reverse("account:worker-calendar-day", kwargs={"pk": self.worker_normal.id, "date": date})
                )
                self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_calendar_day_not_found_worker(self):
        for user in self.user_role_all:
            with self.subTest(user=user):
                ...
                date = (self.DATETIME_NOW + self.TIMEDELTA_THREE_DAYS).date().strftime("%Y-%m-%d")
                response = self.client.get(
                    reverse("account:worker-calendar-day", kwargs={"pk": self.ONE_HUNDRED, "date": date})
                )
    ...
    def test_get_calendar_month_worker(self):
        for user in self.user_role_all:
            with self.subTest(user=user):
                ...
                response = self.client.get(
                    reverse("account:worker-calendar-month", kwargs={"pk": self.worker_normal.id, "date": date})
                )
    ...
    def test_normal_partial_update_worker(self):
        ...
        response = self.client.patch(
            reverse("account:worker-detail", kwargs={"pk": self.worker_normal.id}),
            data=self.role_manager_data,
            format="json",
        )
