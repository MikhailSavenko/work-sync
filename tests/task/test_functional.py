...
class TaskApiTestCase(ApiTestCaseBase):
    ...
    def setUpTestData(cls):
        ...
        cls.task = TaskFactory(
            creator=cls.worker_manager,
            executor=cls.worker_normal,
            deadline=(cls.DATETIME_NOW + cls.TIMEDELTA_THREE_DAYS),
        )
        cls.task1 = TaskFactory(
            creator=cls.worker_admin,
            executor=cls.worker_normal,
            deadline=(cls.DATETIME_NOW + cls.TIMEDELTA_THREE_DAYS),
        )
        ...
        cls.valid_update_data_task = {
            ...
            "status": "DN",
            "deadline": (cls.DATETIME_NOW + cls.TIMEDELTA_THREE_DAYS + cls.TIMEDELTA_THREE_DAYS).strftime(
                "%Y-%m-%dT%H:%M"
            ),
            "executor": cls.worker_normal2.id,
        }
        ...
        cls.valid_partial_upd_status_data = {"status": "DN"}
    ...
    def test_manager_create_input_valid_data_task(self):
        ...
        try:
            ...
            self._assert_task_check_input_data_with_db(
                db_qs=task_in_db, task_data=task_data, worker=self.worker_manager
            )
        except Task.DoesNotExist:
            self.fail(
                f"Task with ID {created_task_id} was not found in the database after manager creation. Response: {response.json()}"
            )
    ...
    def test_admin_create_input_valid_data_task(self):
        ...
        try:
            ...
            self.fail(
                f"Task with ID {created_task_id} was not found in the database after admin creation. Response: {response.json()}"
            )
        ...
        for user in [self.user_admin, self.user_manager]:
            with self.subTest(user=user):
                ...
                response = self.client.post(
                    reverse("task:tasks-list"), data=self.deadline_in_the_past_data, format="json"
                )
                self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_update_manager_or_admin_owner_input_valid_data_task(self):
        params_user_task = {self.user_manager: self.task.id, self.user_admin: self.task1.id}
        for user, task in params_user_task.items():
            with self.subTest(user=user, task=task):
                self.client.force_authenticate(user=user)
                response = self.client.put(
                    reverse("task:tasks-detail", kwargs={"pk": task}), data=self.valid_update_data_task, format="json"
                )
                self.assertEqual(response.status_code, status.HTTP_200_OK)
                task_data = response.json()
                created_task_id = task_data["id"]
                try:
                    task_in_db = Task.objects.get(id=created_task_id)
                    self._assert_task_check_input_data_with_db(
                        db_qs=task_in_db, task_data=task_data, worker=user.worker
                    )
    ...
    def test_update_manager_or_admin_not_owner_input_valid_data_task(self):
        params_user_task = {self.user_manager1: self.task.id, self.user_admin1: self.task1.id}
        for user, task in params_user_task.items():
            with self.subTest(user=user, task=task):
                self.client.force_authenticate(user=user)
                response = self.client.put(
    ...
    def test_update_normal_no_owner_task(self):
        self.client.force_authenticate(user=self.user_normal)
        response = self.client.put(
            reverse("task:tasks-detail", kwargs={"pk": self.task.id}), data=self.valid_update_data_task, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_partial_upd_normal_no_owner_task(self):
        ...
        response = self.client.patch(
            reverse("task:tasks-detail", kwargs={"pk": self.task.id}),
            data=self.valid_partial_upd_status_data,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_partial_upd_manager_or_admin_owner_input_valid_field_status_data_task(self):
        ...
        for user, task in params_user_task.items():
            with self.subTest(user=user, task=task):
                ...
                response = self.client.patch(
                    reverse("task:tasks-detail", kwargs={"pk": task}),
                    data=self.valid_partial_upd_status_data,
                    format="json",
                )
                self.assertEqual(response.status_code, status.HTTP_200_OK)
                ...
                try:
                    ...
                    self.fail(
                        f"Task with ID {created_task_id} was not found in the database after {user.worker.role} partial updated. Response: {response.json()}"
                    )
        ...
        for user, task in params_user_task.items():
            with self.subTest(user=user, task=task):
                ...
                try:
                    ...
                    self.fail(
                        f"Task with ID {created_task_id} was not found in the database after {user.worker.role} partial update. Response: {response.json()}"
                    )
    ...
    def test_update_no_valid_data_task(self):
        ...
        for user, task in params_user_task.items():
            with self.subTest(user=user, task=task):
                ...
                response = self.client.put(
                    reverse("task:tasks-detail", kwargs={"pk": task}),
                    data=self.no_valid_update_data_task,
                    format="json",
                )
                self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class CommentApiTestCase(ApiTestCaseBase):
    ...
    def setUpTestData(cls):
        ...
        cls.valid_data_comment = {"text": "Super text comment"}
        cls.valid_data_comment_another = {"text": "Another text comment"}
        cls.invalid_data_comment_empty_json = {}

        cls.invalid_data_comment_text_list = {"text": ["Another text comment"]}
        ...
        for user in self.user_role_all:
            with self.subTest(user=user):
                ...
                response = self.client.get(
                    reverse("task:comment-detail", kwargs={"task_pk": self.task.id, "pk": self.comment.id})
                )
    ...
    def test_get_detail_not_found_comment(self):
        ...
        for user, task_pk, comment_pk in input_sub_test_value:
            with self.subTest(user=user, task_pk=task_pk, comment_pk=comment_pk):
                self.client.force_authenticate(user=user)
                response = self.client.get(
                    reverse("task:comment-detail", kwargs={"task_pk": task_pk, "pk": comment_pk})
                )
    ...
    def test_get_detail_no_valid_task_pk_comment(self):
        ...
        for user, no_valid_task_pk, comment_pk in input_sub_test_value:
            with self.subTest(user=user, task_pk=no_valid_task_pk, comment_pk=comment_pk):
                self.client.force_authenticate(user=user)
                response = self.client.get(
                    reverse("task:comment-detail", kwargs={"task_pk": no_valid_task_pk, "pk": comment_pk})
                )
    ...
    def test_create_valid_data_comment(self):
        for user in self.user_role_all:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.post(
                    reverse("task:comment-list", kwargs={"task_pk": self.task.id}),
                    data=self.valid_data_comment,
                    format="json",
                )
    ...
    def test_create_invalid_data_comment(self):
        for user in self.user_role_all:
            with self.subTest(user=user):
                ...
                response = self.client.post(
                    reverse("task:comment-list", kwargs={"task_pk": self.task.id}),
                    data=self.invalid_data_comment_empty_json,
                    format="json",
                )
    ...
    def test_create_not_found_task_comment(self):
        for user in self.user_role_all:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.post(
                    reverse("task:comment-list", kwargs={"task_pk": self.ONE_HUNDRED}),
                    data=self.valid_data_comment,
                    format="json",
                )
    ...
    def test_create_invalid_task_pk_comment(self):
        for user in self.user_role_all:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.post(
                    reverse("task:comment-list", kwargs={"task_pk": self.SOME_STR}),
                    data=self.valid_data_comment,
                    format="json",
                )
    ...
    def test_partial_upd_valid_data_comment(self):
        ...
        for user, comment_pk in input_sub_test_value:
            with self.subTest(user=user, comment=comment_pk):
                self.client.force_authenticate(user=user)
                response = self.client.patch(
                    reverse("task:comment-detail", kwargs={"task_pk": self.task.id, "pk": comment_pk}),
                    data=self.valid_data_comment_another,
                    format="json",
                )
    ...
    def test_partial_upd_invalid_data_comment(self):
        ...
        for user, comment_pk in input_sub_test_value:
            with self.subTest(user=user, comment=comment_pk):
                ...
                response = self.client.patch(
                    reverse("task:comment-detail", kwargs={"task_pk": self.task.id, "pk": comment_pk}),
                    data=self.invalid_data_comment_text_list,
                    format="json",
                )
    ...
    def test_partial_upd_invalid_task_pk_comment(self):
        for user in self.user_role_all:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.patch(
                    reverse("task:comment-detail", kwargs={"task_pk": self.SOME_STR, "pk": self.comment.id}),
                    data=self.valid_data_comment,
                    format="json",
                )
...
class EvaluationApiTestCase(ApiTestCaseBase):
    ...
    def setUpTestData(cls):
        ...
        cls.task_done = TaskFactory(
            creator=cls.worker_manager,
            status=Task.StatusTask.DONE,
            executor=cls.worker_normal,
            deadline=(cls.DATETIME_NOW + cls.TIMEDELTA_THREE_DAYS),
        )
        cls.task_done1 = TaskFactory(
            creator=cls.worker_admin,
            status=Task.StatusTask.DONE,
            executor=cls.worker_normal1,
            deadline=(cls.DATETIME_NOW + cls.TIMEDELTA_THREE_DAYS),
        )

        cls.task_no_executor = TaskFactory(
            creator=cls.worker_admin,
            status=Task.StatusTask.DONE,
            deadline=(cls.DATETIME_NOW + cls.TIMEDELTA_THREE_DAYS),
        )

        cls.task_status_not_done = TaskFactory(
            creator=cls.worker_admin,
            executor=cls.worker_normal1,
            deadline=(cls.DATETIME_NOW + cls.TIMEDELTA_THREE_DAYS),
        )

        cls.valid_data_evaluation = {"score": 5}
        cls.valid_another_data_evaluation = {"score": 3}
        cls.invalid_data_evaluation = {"score": 100}
    ...
    def test_create_normal_u_valid_data_evaluation(self):
        self.client.force_authenticate(user=self.user_normal)
        response = self.client.post(
            reverse("task:evaluation-list", kwargs={"task_pk": self.task_done.id}),
            data=self.valid_data_evaluation,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_put_normal_u_evaluation(self):
        self.client.force_authenticate(user=self.user_normal)
        response = self.client.put(
            reverse("task:evaluation-detail", kwargs={"task_pk": self.task_done.id, "pk": self.ONE})
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_create_valid_data_evaluation(self):
        ...
        for user, task_pk in input_data_sub_test:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.post(
                    reverse("task:evaluation-list", kwargs={"task_pk": task_pk}),
                    data=self.valid_data_evaluation,
                    format="json",
                )
                self.assertEqual(response.status_code, status.HTTP_201_CREATED)
                ...
                try:
                    ...
                    self._assert_compare_value_with_db_evaluation(
                        db_obj=evaluation_db, evaluation_data=evaluation_data
                    )
                except Comment.DoesNotExist:
                    self.fail(
                        f"Evaluation with ID {evaluation_id} does not exist in DB after created {user.worker.role}! :O"
                    )

    def test_create_not_found_task_pk_evaluation(self):
        input_data_sub_test = ((self.user_manager, self.ONE_HUNDRED), (self.user_admin, self.ONE_HUNDRED))
        for user, task_pk in input_data_sub_test:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.post(
    ...
    def test_create_invalid_data_evaluation(self):
        ...
        for user, task_pk in input_data_sub_test:
            with self.subTest(user=user):
                ...
                response = self.client.post(
                    reverse("task:evaluation-list", kwargs={"task_pk": task_pk}),
                    data=self.invalid_data_evaluation,
                    format="json",
                )
                self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_create_conflict_hasnt_executor_evaluation(self):
        for user in self.two_role:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.post(
                    reverse("task:evaluation-list", kwargs={"task_pk": self.task_no_executor.id}),
                    data=self.valid_data_evaluation,
                    format="json",
                )
    ...
    def test_create_conflict_task_status_not_done_evaluation(self):
        for user in self.two_role:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.post(
                    reverse("task:evaluation-list", kwargs={"task_pk": self.task_status_not_done.id}),
                    data=self.valid_data_evaluation,
                    format="json",
                )
                self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
    ...
    def test_partial_upd_valid_data_evaluation(self):
        input_data_sub_test = ((self.user_admin, self.task_done1.id), (self.user_manager, self.task_done.id))
        for user, task_pk in input_data_sub_test:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                ...
                patch_response = self.client.patch(
                    reverse("task:evaluation-detail", kwargs={"task_pk": task_pk, "pk": response_post_data["id"]}),
                    data=self.valid_another_data_evaluation,
                    format="json",
                )
                ...
                try:
                    ...
                    self.fail(
                        f"Evaluation with ID {evaluation_id} does not exist in DB after created and partial upd {user.worker.role}! :O"
                    )
    ...
    def test_partial_upd_invalid_task_pk_evaluation(self):
        ...
        for user, task_pk in input_data_sub_test:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.patch(
                    reverse("task:evaluation-detail", kwargs={"task_pk": task_pk, "pk": self.ONE}),
                    data=self.valid_data_evaluation,
                    format="json",
                )
    ...
    def test_partial_upd_not_found_evaluation_pk_evaluation(self):
        ...
        for user, task_pk in input_data_sub_test:
            with self.subTest(user=user):
                self.client.force_authenticate(user=user)
                response = self.client.patch(
                    reverse("task:evaluation-detail", kwargs={"task_pk": task_pk, "pk": self.ONE_HUNDRED}),
                    data=self.valid_data_evaluation,
                    format="json",
                )
                self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
